...
// --------------------------------------------------------------------------
// Trust material (plan section 5.4). M1-M8 are BYTE-IDENTICAL to
// `trust_material.py`'s `_MSG_*` constants, and that claim is measured rather
// than asserted: `tests/fixtures/trust-material-messages.json` is generated by
// the Python side and read back by `messages.test.ts`. Edit one side and the
// other goes red.
//
// M9, M10 and M11 have no Python twin. They refuse three ways of presenting
// bytes that only exist in JavaScript — a multi-byte view over the same
// memory, a buffer shared with another agent, a buffer that has been detached
// or resized out from under its view — and Python's `bytes` has no analogue of
// any of them. A message with no twin is not a parity failure; a message with
// a twin that says something else is.
//
// The subject slot is filled with 'trust store' or 'key manifest' and nothing
// else: no part of the rejected value is ever formatted in, so a hostile
// document cannot choose what the verifier prints.
// --------------------------------------------------------------------------

export const notBytes = (what: string): string =>
  `${what} must be supplied as serialized bytes; a live object is not accepted`
export const tooLarge = (what: string): string => `${what} exceeds the admission ceiling`
export const notParsed = (what: string): string =>
  `${what} must be a parsed snapshot produced by this library; a live object is not accepted`
export const notObject = (what: string): string => `${what} document must be a JSON object`
export const unknownMember = (name: string): string =>
  `trust store document has an unknown member ${pyStage2StringRepr(name)}`
export const memberShape = (member: string, expected: string): string =>
  `trust store member ${pyStage2StringRepr(member)} must be ${expected}`
export const unparsable = (what: string, reason: string): string =>
  `${what} could not be parsed: ${reason}`
export const notCanonical = (what: string, reason: string): string =>
  `${what} is outside the canonical profile: ${reason}`
export const notByteView = (what: string): string =>
  `${what} must be supplied as a byte view (Uint8Array, Int8Array or Uint8ClampedArray); a DataView or a multi-byte typed array is not accepted`
export const sharedMemory = (what: string): string => `${what} must not be backed by shared memory`
export const detachedBuffer = (what: string): string =>
  `${what} is backed by a detached or out-of-bounds buffer`

export const ERR = {
  // M3, pre-rendered for the two subjects the doors use. The doors compare
  // against a constant rather than calling the builder, so the constant is
  // built here once from the same source.
  TRUST_STORE_NOT_PARSED: notParsed('trust store'),
  KEY_MANIFEST_NOT_PARSED: notParsed('key manifest'),
