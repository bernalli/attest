...
import {
  verifyRecordSignatureData as verifyTransferRecordSignatureData,
...
// predicate reads it: on a live object the entry reads below
// (`entry['status']`, `entry['valid_to']`) would go through whatever accessor
// that object defined, while the signature check read own data — so the same
// manifest could be authentic and lying at once, and a record signed after the
// key expired verified. A HANDLE removes the object; this unwraps it. In-package
// callers that already hold the tree use `verifyRecordSignatureData`, so the
// unwrap is one per public call and never one per record.
...
// `verifyRecordSignature`'s body, over the snapshot TREE.
// Second precondition on top of the public one: `keyManifest` is a tree from
// `manifestData`/`storeData`, so it holds only what `loadsStrict` produces and
// the reads below cannot be steered. Calling this with a live object reopens
// the class the handle exists to close. Exported, not module-local,
 ...
 * 2. `transfer.verifyRecordSignatureData(record, issuerManifest)` — the
...
// below uses `verifyRecordSignatureData`, the variant that does not
...
export function classifyRevocation(
  ...
  // verify.py's _resolve_transfer_backing and _classify_revocation.
  const manifestOk = manifestSignatureIsAuthentic(issuerManifest)
  // Tree variant, hoisted for the same reason as the transfer rail:
  // this map runs up to MAX_REVOCATION_RECORDS times against ONE manifest.
  const auth: boolean[] = admittedView.map((r) => { const o = asObject(r); return manifestOk && o !== null && verifyRecordSignatureData(o, issuerManifest) })
