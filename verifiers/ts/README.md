...
This runs `test/conformance.test.ts`, which discovers every leaf directory under [`docs/spec/vectors/`](../../docs/spec/vectors/) (any directory containing an `expected.json`, walked recursively so multi-part vectors like `07-unicode-canon/a-...` and `17-binding-proven/b-...` are included) — **158 leaves across 40 groups** — and routes each leaf to the surface its files name. A leaf belongs to exactly one of four:

- **`verify()`** (130 leaves), for every leaf naming none of the three files below: envelope bytes, trust store, revocation view, disclosure, and the Stage 2/3/4 evidence channels. Two of those channels are files a leaf opts into — `transfer-view.json` (v0.2 §17) and `grant-view.json` (v0.2 §18.4, the sunset-grant evidence object `{grant[, later_grants][, declarations][, anchor]}`). Supplying `grant-view.json` *is* Stage 4's capability gate: a leaf without it gets `grant`/`grant_trust` at `not_checked`, which is exactly what group 37's own v0.1 negative control pins.
