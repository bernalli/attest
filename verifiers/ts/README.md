...
- **`verifyRedemption`** (v0.2 §18.7), for a leaf carrying `redemption.json` — four leaves, the one share a guard test pins. The question these ask involves no receipt and no grant document at all, only whether a holder's audience-bound proof is good for *this* custodian, so they carry no envelope or trust store; their result shape is a single `{"verified": bool}`, and every negative one must come back `false` rather than throw.
