...
- **A verifier the buyer can keep: one HTML file, opened by double-click.**
  This is the first release to publish it. `attest-verifier.html` is attached to
  the GitHub Release together with its `.sha256`, and checks a receipt entirely
  offline — no server, no runtime to install, and nothing fetched at any point,
  a property enforced when it is built by an allowlist over the document's own
  start-tag stream and checked against real browser engines. It is not part of
  either published package, and neither `attest-receipts` nor `attest-verifier`
  changes because of it: it is for the reader who has a receipt and no toolchain.
  Download it beside its checksum, confirm it with
  `sha256sum -c attest-verifier.html.sha256`, and open the file.
