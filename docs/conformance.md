...
- `--adapter` is a command **template** containing the literal placeholder
  ...
  shell string).
- `--subset` selects `v0.1` (63 leaves) or `v0.2` (all leaves, currently 216) —
  see §4.
...
```
CONFORMANT (v0.2): 216/216 leaves pass — corpus revision <hex12>
...
- **v0.2** — every leaf in the corpus (currently 216). Measures conformance
...
- **v0.1** — the 63-leaf subset: every leaf whose top-level group directory's
  ...
  profile) is measured against this subset, not against all 216.
