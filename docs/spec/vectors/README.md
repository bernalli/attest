...
| 23c | `revocation-refund-window/c-window-end-unrepresentable` | The same receipt shape issued at `9999-12-20T00:00:00Z`: `issued_at + revocation_window_days` (14) lands past `9999-12-31T23:59:59Z` and so cannot be expressed in the timestamp form the specification compares, with an otherwise genuinely authenticated, matching revocation record in hand — `errors` gains `"refund window is outside the representable timestamp range"`, `revocation: "unknown"`, `ok: false`. Added as an implementation-parity leaf; adopted as normative by v0.1 §12.2 rev 18 (2026-09-07), which is what it now pins. |
| 23d | `revocation-refund-window/d-unrepresentable-no-matching-record` | The same unrepresentable-deadline receipt, with a non-empty view whose only record is authenticated and concerns a DIFFERENT receipt — same error, `revocation: "unknown"`, `ok: false`. The view is non-empty and within §12.4's ceiling, so the diagnostic is owed independently of whether a record matches; this leaf is what separates §12.2's fourth row from a narrower reading owing it only where a record matches, which leaves 23c passing and answers `not_revoked_as_of:<T>` with `ok: true` here. Added by v0.1 rev 18 (2026-09-07). |
...
| 28o | `o-malformed-signature-logged` | Genuine log evidence for a receipt whose 64-byte signature does not verify, with the signing key `active` and untouched — `signature: "invalid"` carrying `signature verification failed`, never `compromised`, while `transparency`/`corroboration` stay `logged` and `ok` is `false`. Pairs with 28i: same conclusion, different cause. Neither exercises §8's admission rule — a vector is a verifier fixture and never reaches `tlog.encode_entry`. |
...
| 35l | `l-countersigner-compromised-old-receipt-revives` | The old receipt's transfer record and `transferred` revocation are genuinely issuer-signed and holder-authorized, but by a SECOND hybrid issuer key that a later manifest version marks `compromised` while the receipt's own signer stays `active` — both side-documents lose authentication (v0.1 §12.1, §17.1) and the old receipt revives: `revocation: "unknown"`, `ok: true`. The `unknown` is this view's: it holds no other authenticated statement-status record, and one would make the same fixture report `not_revoked_as_of:<T>` (v0.1 §12.3), still `ok: true`. Pins threat model TM-80's deliberate exclusion. |
| 35m | `m-compromised-countersigner-cannot-back-the-transfer` | `35l` with ONE signature reversed: the `transferred` revocation is signed by the receipt's own `active` key, so it authenticates and the transfer claim IS consulted; the transfer record — byte-for-byte `35l`'s, still countersigned by the `compromised` key — is refused at §17.3's key-authorization gate and the revocation is left unbacked: `revocation: "invalid_revocation_ignored"`, warning `transferred_revocation_unbacked`, `ok: true`. Pins the half of TM-80 that `35l` cannot observe, since there the revocation fails first and the record's own refusal is never reached. |
...
v0.1 §12.3 (2026-08-26 amendment): only a record that is both AUTHENTICATED and
carries a registered revocation-statement `status` may drive the freshness anchor
`T`. Leaves `a`/`b` carry a single issuer-signed record with `revoked_at:
"2099-01-01T00:00:00Z"`, covered by the signer's open-ended validity window, and an
unregistered `status`. Leaf `c` is the sibling malformation on the OTHER required
field: a record with a REGISTERED `status: "revoked"` whose own `receipt_id` is not
ULID-shaped, which fails `revocation.verify_record_signature`'s type/format gate
(12.1) before the status is ever reached — genuine issuer signature notwithstanding,
it never authenticates, so it can never set `T` either.
