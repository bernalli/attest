...
| 23c | `revocation-refund-window/c-window-end-unrepresentable` | The same receipt shape issued at `9999-12-20T00:00:00Z`: `issued_at + revocation_window_days` (14) walks off the representable `datetime` range, with an otherwise genuinely authenticated revocation record in hand — `errors` gains `"refund window is outside the representable timestamp range"`, `revocation: "unknown"`, `ok: false`. |
