...
The table describes the revocation-by-class dispatch after the receipt has passed the preceding verification gates and the supplied view has passed §12.4's record-count ceiling. An oversized view follows §12.4 instead: it is not evaluated and reports that section's error, without owing the refund-window diagnostic. Within the admitted non-empty view, the unrepresentable-deadline diagnostic does not depend on finding or authenticating a matching record. This is an outcome requirement, not a requirement to compute the deadline before record authentication. The v0.2 §17.3 fallthrough described below may subsequently change the reported revocation value, but it MUST preserve the error.
...
> **Amendment note (2026-09-07, rev 18): a refund window whose end cannot be represented.** The first three rows were written on the assumption that `issued_at + revocation_window_days` is itself always a timestamp. It is not: `revocation_window_days` is a schema-bounded integer (1–3,650) and `issued_at` may name any instant this specification can express, so a receipt issued in the last days of year 9999 has a deadline past the end of the four-digit-year range. The fourth row changes nothing either reference implementation does on canonical UTC timestamps — it states a case the table never contemplated, and which the table's silence left to each implementation to answer alone.
>
> **Why `unknown` with an error.** The sum has a mathematical value, but this amendment declines to use a deadline outside the required four-digit-year timestamp form. Two alternative implementations were measured on leaves 23c and 23d. Saturating the deadline at `9999-12-31T23:59:59Z` reports `revoked`, `ok: false` for 23c and `not_revoked_as_of:<T>`, `ok: true` for 23d. For canonical timestamps in the specified range, saturation preserves the comparison with every authenticated record: those record times precede an overflowing mathematical deadline. It does not erase a matching revocation. Treating the deadline as absent instead reports `invalid_revocation_ignored`, `ok: true` for 23c and `not_revoked_as_of:<T>`, `ok: true` for 23d. The fourth row deliberately requires refusal in both cases whenever the admitted non-empty view engages this dispatch. Its error, rather than the `unknown` literal alone, forces `ok: false` under §11.1. A later v0.2 §17.3 transferred-class fallthrough may relabel the result but MUST preserve that error.
>
