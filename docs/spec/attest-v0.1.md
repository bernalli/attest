...
**What `binding: "proven"` establishes, and what it does not.** The issuer records the binding values in the signed payload; this does not imply knowledge of a private key supplied by a buyer. The identifier reaches the issuer at checkout; the salt is generated per receipt by the issuer (§8.1); `buyer.pubkey` is a value the issuer writes into the payload it then signs (§5.3). The receipt payload carries no buyer signature accepting the purchase, and nothing in this specification obliges the identifier or the key a receipt names to be the buyer's. A `binding: "proven"` result (§11.1) therefore establishes exactly one thing: **the party presenting the disclosure holds a secret that reproduces a value the issuer wrote into the payload** — the `(identifier, salt)` pair behind `buyer.commitment` on the §8.1 path, the private key of `buyer.pubkey` on the §8.2 path. It does **not** establish that the buyer took part in issuance, agreed to the receipt, or was ever asked: an issuer can fabricate a receipt using an identifier and salt it knows and a key it generated itself, then produce a matching disclosure. This does not let the issuer answer a fresh challenge for an already-issued receipt whose recorded public key has a private half held only by another party. §8.2's prohibition on treating `buyer.pubkey` equality across two receipts as proof of buyer identity is one case of this limit, not the whole of it. `binding: "not_proven"` is equally narrow in the other direction: it records that no proof was produced or that the one produced did not verify, never that the presenting party is not the buyer. Whether the party a receipt names in fact bought the work is an issuer-honesty question this specification does not adjudicate (§2, §6.1; threat model TM-05, TM-78); the rendering constraint that follows from this paragraph is stated in §11.1.
...
> **Amendment note (2026-09-05, v0.1 rev 17).** `binding` reports possession of a binding secret, never participation of a buyer: §8 states what each of its two paths proves and what it does not, and the provenance of the signed binding values is issuer-asserted. A conforming rendering surface MUST NOT present `binding: "proven"` as evidence that the person, account, or address the receipt names made the purchase, and MUST NOT present `binding: "not_proven"` as evidence that the presenting party is not the buyer — that value records only that no proof was produced or that the one produced did not verify. Nothing about how the component is computed changes and `binding` stays outside the `ok` predicate: this bounds what a surface may claim from a result, not the result itself.
...
| `refund_window`, deadline NOT representable (rev 18) | **Not evaluated.** Where `issued_at + revocation_window_days` falls later than `9999-12-31T23:59:59Z` the deadline cannot be expressed in the ISO-8601 UTC form this specification requires of every timestamp it compares (§5.1, §12), so the window has no end against which `revoked_at` can be placed. A verifier MUST NOT report `revoked` on the strength of a deadline it could not compute, and MUST NOT treat a record as falling outside a window that has no end. It MUST report `revocation: "unknown"`, MUST record the error `refund window is outside the representable timestamp range`, and `ok` is thereby `false` (§11.1). | **The same**, deliberately: the deadline is computed before any record is examined, so a non-empty `revocation_view` produces the same `unknown`, the same error and the same `ok: false` whether or not any record in it concerns this receipt. A view that is absent or empty is not evaluated at all and is unaffected — `revocation: "unknown"`, no error, `ok` untouched. |
...
> **Amendment note (2026-09-07, rev 18): a refund window whose end cannot be represented.** The first three rows were written on the assumption that `issued_at + revocation_window_days` is itself always a timestamp. It is not: `revocation_window_days` is a schema-bounded integer (1–3,650) and `issued_at` may name any instant this specification can express, so a receipt issued in the last days of year 9999 has a deadline past the end of the four-digit-year range. The fourth row changes nothing either reference implementation does — it states a case the table never contemplated, and which the table's silence left to each implementation to answer alone.
>
> **Why `unknown` with an error, and not either alternative.** Three readings of an uncomputable deadline exist and only this one fails closed. *Saturating* the deadline at the last representable instant places every matching record inside the window, so an arithmetic accident revokes a receipt on the strength of a comparison nobody performed. *Treating the deadline as absent* — a window no record can fall inside — is worse in the other direction, and measurably so: under that reading the reference implementation verifies vector 23c **green**, `ok: true`, with a genuinely authenticated, matching `status: "revoked"` record in hand, reporting it as `invalid_revocation_ignored` and warning that the record fell "outside refund window" — outside a window that has no outside. The rule above says instead the one thing that is true, that the window could not be evaluated, and records it as an error. `ok` is `false` because §11.1 makes it `false` for any non-empty `errors`: **that**, not the literal in `revocation`, is what makes this fail closed, and it is why a later rule that renames the reported value cannot reopen the receipt — v0.2 §17.3's transferred-class fallthrough may rename it, and the error survives the rename.
>
> **Where the boundary lies, and why it is stated in this specification's own terms.** A deadline at or before `9999-12-31T23:59:59Z` is representable and is evaluated exactly as the second row prescribes; anything later is not. Measured on both reference implementations with `revocation_window_days: 14`: `issued_at: "9999-12-17T00:00:00Z"`, whose deadline is `9999-12-31T00:00:00Z`, reports `revoked`; `issued_at: "9999-12-18T00:00:00Z"`, whose deadline is `10000-01-01T00:00:00Z`, reports `unknown` with the error. The bound is the ISO-8601 UTC form this specification requires of every timestamp it compares, never an implementation's own date type — two verifiers whose languages' calendars end in different millennia must still refuse at the same instant, or they disagree on unchanged bytes. That divergence is not hypothetical: it is the gap the corpus leaf named below was added to close.
>
> **Pinned by two leaves, and the second is not redundant.** [`23-revocation-refund-window`](vectors/23-revocation-refund-window/)'s `c-window-end-unrepresentable` carries a matching record; `d-unrepresentable-no-matching-record` carries an authenticated record for a different receipt. Only the second distinguishes this rule from a narrower one owing the diagnostic solely where a record matches — measured by mutation on both implementations, that narrower reading leaves `23c` passing and fails `23d` alone, where it certifies a receipt whose window could not be evaluated, `ok: true`, on the strength of an unrelated record's freshness.
...
- **2026-09-07 (rev 18)**: §12.2 gains a fourth row for the case its table never contemplated — a `refund_window` receipt whose `issued_at + revocation_window_days` falls past `9999-12-31T23:59:59Z` and so cannot be expressed in the timestamp form this specification compares. The deadline having no value, the verifier MUST report `revocation: "unknown"` with the error `refund window is outside the representable timestamp range`, which makes `ok` false by §11.1; the existing `refund_window` row is scoped to a representable deadline, and the boundary is stated in the specification's own timestamp form rather than any implementation's date type. Both reference implementations already behaved this way and neither changes; what changes is the specification, which previously prescribed `revoked` for a matching record and left an unevaluated window certifiable where none matched. Behavior changes for a verifier faithful to the old text in exactly one direction, toward refusing: a receipt with an unrepresentable deadline and a non-empty revocation view carrying nothing about it was certified before and is refused now, and attest-versioning.md §2 gains that refusal as its thirteenth sanctioned instance. A view that is absent or empty is never evaluated and is untouched, as is every receipt whose deadline is representable. — vectors: 23-revocation-refund-window

- **2026-09-05 (rev 17)**: §8 gains a closing paragraph stating what `binding: "proven"` establishes — possession, by the party presenting the disclosure, of a secret that reproduces a value the issuer wrote into the payload — and what it does not: participation of the buyer at issuance. The issuer chooses the binding values it signs and can fabricate a receipt using secrets it knows; this does not grant it the private half of a buyer-generated key recorded in an existing receipt; §8.2's existing prohibition on reading `buyer.pubkey` equality across two receipts as proof of buyer identity is placed as one case of that limit rather than the whole of it, and `binding: "not_proven"` is stated to be equally narrow in the other direction. §11.1 gains the rendering constraint that follows: a conforming rendering surface MUST NOT present `binding: "proven"` as evidence that the person the receipt names made the purchase, nor `binding: "not_proven"` as evidence that the presenting party is not the buyer. Nothing on the wire moves — no field, preimage, algorithm step, result value or `ok` predicate changes, and no receipt verifies differently — because the revision states a property both mechanisms already had and bounds what a surface may claim from it; the case is catalogued as threat model TM-78, whose verdict stays out of scope with the claim narrowed rather than the fabrication prevented. — vectors: none
