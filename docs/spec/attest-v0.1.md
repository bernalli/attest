...
**What `binding: "proven"` establishes, and what it does not.** Every input to both mechanisms in this section is issuer-chosen. The identifier reaches the issuer at checkout; the salt is generated per receipt by the issuer (§8.1); `buyer.pubkey` is a value the issuer writes into the payload it then signs (§5.3). Neither path carries anything the buyer signed, and nothing in this specification obliges the identifier or the key a receipt names to be the buyer's. A `binding: "proven"` result (§11.1) therefore establishes exactly one thing: **the party presenting the disclosure holds a secret that reproduces a value the issuer wrote into the payload** — the `(identifier, salt)` pair behind `buyer.commitment` on the §8.1 path, the private key of `buyer.pubkey` on the §8.2 path. It does **not** establish that the buyer took part in issuance, agreed to the receipt, or was ever asked: an issuer holds every input to both paths for every receipt it has issued, so it can produce, for any of them, a disclosure that yields `proven` against an identifier or a key of its own choosing. §8.2's prohibition on treating `buyer.pubkey` equality across two receipts as proof of buyer identity is one case of this limit, not the whole of it. `binding: "not_proven"` is equally narrow in the other direction: it records that no proof was produced or that the one produced did not verify, never that the presenting party is not the buyer. Whether the party a receipt names in fact bought the work is an issuer-honesty question this specification does not adjudicate (§2, §6.1; threat model TM-05, TM-78); the rendering constraint that follows from this paragraph is stated in §11.1.
...
- **2026-09-05 (rev 17)**: §8 gains a closing paragraph stating what `binding: "proven"` establishes — possession, by the party presenting the disclosure, of a secret that reproduces a value the issuer wrote into the payload — and what it does not: participation of the buyer at issuance. Every input to both paths is issuer-chosen (the identifier arrives at checkout, the salt is generated per receipt by the issuer, `buyer.pubkey` is written into the payload the issuer signs), so an issuer can produce, for any receipt it has issued, a disclosure that yields `proven` against an identifier or a key of its own choosing; §8.2's existing prohibition on reading `buyer.pubkey` equality across two receipts as proof of buyer identity is placed as one case of that limit rather than the whole of it, and `binding: "not_proven"` is stated to be equally narrow in the other direction. §11.1 gains the rendering constraint that follows: a conforming rendering surface MUST NOT present `binding: "proven"` as evidence that the person the receipt names made the purchase, nor `binding: "not_proven"` as evidence that the presenting party is not the buyer. Nothing on the wire moves — no field, preimage, algorithm step, result value or `ok` predicate changes, and no receipt verifies differently — because the revision states a property both mechanisms already had and bounds what a surface may claim from it; the case is catalogued as threat model TM-78, whose verdict stays out of scope with the claim narrowed rather than the fabrication prevented. — vectors: none
