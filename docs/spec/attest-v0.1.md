...
This amendment fixes a floor, not a ceiling. A conforming importer MUST accept a container in which none of the three quantities below exceeds its floor value — measured on every value the container DECLARES (the end-of-central-directory entry counters, each central-directory record's uncompressed size) and on every value it PRODUCES when read (the bytes each member actually inflates to). A container that declares one thing and produces another is within the floor on neither reading, and is malformed besides:
...
This section has no conformance vector, for the reason §14.1 already gives: the vector corpus has no container surface (§15 feeds `verify()` and its sibling audit surfaces). Its pre/post behavior is pinned by the container bench under `tests/container-corpus/`, against which the shared container reading is judged in both reference implementations and whose leaves declare the caps they were read under; each reference importer is held to the bench from the revision in which it reads through that shared reading.
