...
Steps 1 to 3 read only the signed payload and need no evidence at all; they run before any short-circuit that depends on what a caller chose to supply, so a defect visible in the receipt itself is never masked by missing evidence.
...
8. **Scope coverage, and it is a gate.** If the effective grant's `scope` does not cover this receipt's `work.artifact_series`/`work.artifacts` (§5.4) under the coverage rule above → warning `grant_scope_uncovered` and `grant: "dormant"`, without evaluating either activation path. A grant scoped to a broader catalogue than this receipt's artifacts DOES cover it and passes here; what fails is a grant that simply does not concern these artifacts, and for that receipt there is no permission to become exercisable. Reporting `activated` on an uncovered receipt would tell the holder they may redeem something the grant never spoke about, and would contradict the custodian precondition of §18.7, which requires the served artifact to be inside the effective grant's scope.
9. **Declaration path.** Any supplied declaration that authenticates (signed by the publisher or by a successor listed in the EFFECTIVE grant) and covers its scope → `grant: "activated"`, plus `grant_activated_by_successor` where the signer was a successor. A supplied declaration that fails authentication or coverage is ignored with warning `grant_declaration_ignored` and never honored.
10. **Fixed-date path.** `"fixed-date"` in the effective grant's `modes`, `fixed_date` non-null, and a valid anchored attestation with `T >= fixed_date` → `grant: "activated"`. The mode declared but the proof absent or insufficient → warning `grant_unanchored`.
