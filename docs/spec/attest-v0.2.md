...
- every `sha256` in the receipt's `work.artifacts[]` appears in `grant.scope.artifacts`.

Either alone suffices. In particular, a grant scoped purely by artifact hash (`artifact_series: null`) covers a receipt that names exactly those artifacts, EVEN IF that receipt also carries a `work.artifact_series` — requiring the series to match here, as declaration coverage does, would deny a buyer a grant that demonstrably names their own files. Conversely, a receipt carrying only a series the grant does not name is uncovered: the verifier will not resolve a series into hashes to reach a wider verdict, because that resolution depends on evidence outside the receipt, and reaching further to say `activated` is exactly the direction §18.4 forbids.
