...
Sixteen of the twenty-three leaves end somewhere other than `activated`, and the imbalance is deliberate: §18.4's failure asymmetry is normative, and a false `activated` is the single failure that would discredit the instrument. Every leaf's `signature`, `schema`, `revocation`, `binding`, `trust` and `ok` read exactly as they would with no grant evidence in sight — that is D6 as a vector rather than as prose.
