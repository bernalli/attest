...
def test_build_verify_record_roundtrip() -> None:
    record = revocation.build_record(RECEIPT_ID, "revoked", "2026-07-03T00:00:00Z", KP, KID)
    assert revocation.verify_record(record, kmh(_key_manifest()))
...
def test_self_inconsistent_key_manifest_false() -> None:
    record = revocation.build_record(RECEIPT_ID, "revoked", "2026-07-03T00:00:00Z", KP, KID)
    key_manifest_doc = _key_manifest()
    key_manifest_doc["keys"][0]["status"] = "compromised"  # breaks key_manifest's own signature
    assert not manifests.verify_key_manifest(kmh(key_manifest_doc))
    assert not revocation.verify_record(record, kmh(key_manifest_doc))
...
def test_record_signed_by_compromised_key_is_rejected() -> None:
    ...
    key_manifest_doc = _manifest_with(
        manifests.key_entry(
...
def test_record_revoked_at_before_key_valid_from_is_rejected() -> None:
    ...
    key_manifest_doc = manifests.build_key_manifest(
        ISSUER, 1, "2026-06-01T00:00:00Z", entries, KP, KID
    )
...
def test_record_revoked_at_within_key_window_is_accepted() -> None:
    ...
    assert revocation.verify_record(record, kmh(key_manifest_doc))
...
def test_a_genuinely_signed_record_with_a_malformed_receipt_id_does_not_authenticate() -> None:
    ...
    key_manifest_handle = kmh(_key_manifest())
    for receipt_id in malformed:
        record = _signed_record_with_receipt_id(receipt_id)
        assert not revocation.verify_record_signature(record, key_manifest_handle), receipt_id
...
def test_a_well_formed_receipt_id_still_authenticates() -> None:
    ...
    record = _signed_record_with_receipt_id(RECEIPT_ID)
    assert revocation.verify_record_signature(record, kmh(_key_manifest()))
