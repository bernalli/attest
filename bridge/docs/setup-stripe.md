...
> **The Ledger database is a secret.** While the bridge runs it consists of
> `ledger_path` plus SQLite's `-wal` and `-shm` sidecars; committed receipts
> can still live only in `-wal`. Back it up **encrypted**, using SQLite's
