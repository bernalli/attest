...
| `60773e80b6fd` | `explain.ts` | `can be` | … this page pins no block headers, so a refund-window revocation cannot be proven timely here; the CLI, given an anchor policy, can | Two tools, in one sentence. Negative about THIS artifact — it pins no block headers, so it cannot date a refund-window revocation — and positive about the attest CLI, which can, given an anchor policy. | `grep -n pinnedHeaders site/src/trusted-log.ts  # the shipped ANCHOR_POLICY is { pinnedHeaders: {}, crqcHorizon: null }: nothing pinned. grep -n '"--anchor-policy"' src/attest/cli.py  # declared on the verify subparser (cli.py:4610). Dating a refund-window revocation needs --revocations, --revocation-evidence, --log-keys AND --anchor-policy together; cli.py:3576 warns when the evidence arrives without both pins. The pairing REFUSAL at cli.py:1647 is `manifest compromise-view`'s, not verify's: verify accepts --anchor-policy alone` | **TRUE** | 2026-09-06 |
...
| `64278c3086b1` | `intake.ts` | `can ` | … the file itself is bearer proof: anyone who holds it can produce this | None. States a property of the file format, not something a tool does. | `n/a — no capability asserted` | **NOT-A-TOOL-CLAIM** | 2026-09-05 |
